...
import { saveBlogDetails } from '@/lib/firebase/firebase-service';
    ...
    const handleClick = async (event: any, action: any) => {
        ...
        try {
            if (image && image.size > 512000) {
                ...
                const base64 = image ? await toBase64(image) : "";
                const draft = action === "PUBLISH" ? false : true;
                const docId = await saveBlogDetails(title, quotes, base64, article, userUid, draft, category);
                if (docId) {
    ...
    return (
        <div className="w-full mx-auto my-8 flex flex-col-reverse lg:flex-row items-start justify-around p-2">
            ...
            <ReactQuill modules={modules} theme="snow" value={article} onChange={(value) => {
                ...
                setError("");
                setErrorType("");
            }} className="bg-secondary border-none min-h-screen w-full lg:w-2/3  mt-6 lg:mt-0" />
